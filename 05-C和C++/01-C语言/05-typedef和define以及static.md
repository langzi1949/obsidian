...
#const
比较一下：  `const int *pt;` 和 `int *const pt`

其实记住一个规律： `const`靠近谁就是修饰的谁。。

| 形式       | 语法                                    | 含义                     | 是否修改                   |
| -------- | ------------------------------------- | ---------------------- | ---------------------- |
| **指向常量** | `const int *pt;` 或者  `int const *pt;` | 指向的内容是const            | 指针本身可以修改，但是指针指向的内容不可修改 |
| **常量指针** | `int *const pt;`                      | 指针本身是const，不能将指针指向其他地方 | 指向的内容是可以修改的            |
| **都为常量   | `const int *const pt;`                | 两者都是const的             | 两者都不能改变                |

```cpp
 int a = 89;
int b = 9000;

const int *pt = &a; // 和  int const *pt1 = &a;  是等价的
//*pt = 20;  // 这个是错误的，不能修改指针指向的值 ❌ ❌ ❌

int *const pt1 = &a;
*pt1 = 20;
// pt1 = &b; // 不能修改指针的指向 ❌ ❌ ❌

const int *const pt2 = &b;
// pt2 = &a; 不能修改指针的指向 ❌ ❌ ❌
// *pt2 = 39;  不能修改指针指向的值 ❌ ❌ ❌
```

> [!danger]
> 需要注意的是如果一开始变量就是const的，那么指针必须要为指向的内存是const的
> `cont int a = 1;  `
> `const int *pt = &a;`
 
# 6. `#`和`##`

在C语言的宏定义中，`#`和`##`是两个特殊的预处理运算，它们只在宏替换时起作用。

## 6.1 `#`

`#`运算符用于将宏参数转换为**字符串字面量**。它只能用于带参数的宏定义中，并且必须放在宏参数的前面

作用： <span class="b">将宏参数，转为一个用双引号引起来的字符串</span>

```c
#define PRINT_VAR(x) printf("变量 " #x " 的值是：%d\n", x)
...
PRINT_VAR(some_long_name);
// 预处理后的结果：
// printf("变量 " "some_long_name" " 的值是：%d\n", some_long_name);
```

那么`#x`就等于传入的参数的那个实际变量的名称

## 6.2 `##`

`##` 运算符 用于将两个标记连接在一起，形成新的标记

作用： <span class="b">将##左右两边的宏参数或者普通文本粘合在一起，形成一个新的有效的标识符（如： 变量名、 函数名、 类型名等）</span>
...
// 1. 粘合成变量名
int my_id = 100;
int CONNECT(my, _id) = 200; // 预处理后：int my_id = 200;
printf("%d\n", my_id); // 输出 200
...
#define CALL_LOG(level, msg) log_##level(msg)

CALL_LOG(info, "程序启动");
// 预处理后：log_info("程序启动");

CALL_LOG(error, "出现致命错误");
// 预处理后：log_error("出现致命错误");
```


# 7. 为啥不在头文件中进行函数定义？

其实这个问题是错误的，可以在头文件中进行函数定义，但是有限制，只能定义`static`函数和`inline`函数
为啥只能是上面的两种情况呢？
- **静态函数**，具有**内部链接性**，意味着它的作用域仅限于定义它的那个编译单元(其实就是c文件)，即使它在多个文件中被定义，链接器也会将它们视为不同的、互不干扰的函数
- **内联函数**  inline函数可以被定义多次，编译器通常会在每个调用点尝试将其代码内联展开，从而避免生成单独的符号，说白了就是替换掉里面的函数体

除了以上两个函数定义外，其他普通的函数不能在头文件中定义，因为`#include` 是文本替换的逻辑，如果有多个c文件`#include`同一个函数定义，这样会有多份函数定义在不同的c文件中，导致最后编译器懵逼了，不知道用哪个函数，从而报错：重复定义函数的Error。



# 8. 几个面试题

>  <span class="b">写一个宏，计算结构体中某变量相对于首地址的偏移量，并给出说明</span>
...
我们思考一下，为啥`(struct Stu*)0` 这种强制转换是合法的呢？

- 使用`(struct Stu*)0` 这种转换的方式在C/C++中非常常见而且经典
- `0`是一个特殊的常量
	- 在C/C++中`0`可以**隐式转换**为任何类型的指针，可以表示**空指针**
- 语法层面，是没有任何问题
- 目的层面，它的价值就是在于用作一个<span class="r">假想的基地址， 用于在编译时期计算成员内部分布偏移量</span>
