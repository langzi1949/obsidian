...
<span class="r" style="font-size:1.6rem">那么，我们使用自己定义一个namespace呢？</span>

1. 正常情况下，在头文件中进行变量的声明或者函数的声明
...
2. namespace下的函数的实现，需要用单独的cpp文件来进行处理。

```cpp
#include "testnamespaceh.h"
...
3. 最后使用的时候，编译的时候，需要一起编译，这样才能进行链接。其实这个过程就是跟正常的外部的函数使用是一样的
...
int main(void)
{
    int res = zmg::fuccc(20);
    cout << res << endl;
    return 0;
}
```

最后一起编译上面的两个cpp文件，`g++ -o testnamespace testnamespace.cpp fuccc.cpp`
这样就OK了。

>[!danger]
>如果我们想在namespace下再添加namespace就一层一层的嵌套就可以了。在C++17过后提供了新的方式
>`namespace zmg::eva { ..... }`
...
需要注意的是：C++中如果用C风格的字符串的话，比较两个字符串是否相等，必须要用`strcmp()`函数
```cpp
char str[] = "hello";
str == "hello";  // 这个是错误的 ❌ ❌ ❌ ,因为str是一个指针
```
但是如果用`string`类的话，是可以这么比较的。
...
- libstdc++   它的源码在gcc中的一个目录下 [libstdc++](https://github.com/gcc-mirror/gcc/tree/master/libstdc%2B%2B-v3)
- libc++   这个是在llvm下的一个目录 [libc++](https://github.com/llvm/llvm-project/tree/main/libcxx)
