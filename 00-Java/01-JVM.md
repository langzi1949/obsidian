...
在JDK7之前，JVM在实现`Method Area`的时候是用『永久代』在JVM堆内存中实现的，而现在却是用本地内存来实现，这样大小就由物理内存决定。
...
## 2.5 Metaspace区域会发生OOM吗？什么场景会发生OOM?

首先可以肯定的是Metaspace区域是有可能会发生OOM的，它的表现或者Log输出的信息大致是这样的
...
在JDK8之后Metaspace的区域从JVM中移出到本地内存了，其上限就取决于物理内存的大小，那么，什么情况下会把物理内存也撑爆呢？
搞懂这个问题，我们先捋一下Metaspace中存放了哪些数据。
- 类的全限定名、修饰符、父类与接口的关系（其实就是类的元数据）
- **方法定义**  （方法名、方法返回值、参数类型、字节码指令、异常表等）
- **运行时常量池**
- 类加载器的引用
- JIT编译后的代码缓存
所以针对以上的数据信息，我们可以看出常见的OOM场景是

| 场景               | 原因                              | 典型项目                                       |
| ---------------- | ------------------------------- | ------------------------------------------ |
| 动态生成大量类          | cglib、JDK动态代理等频繁生成代理类           | Spring AOP、Feign                           |
| 类加载器泄露           | 自定义的ClassLoader没有被回收，导致重复加载同一个类 | Web容器的热部署，来加载外部的jar，随着热部署次数增加，旧类不断堆积，最后OOM |
| Spring、框架大量的Bean | 注解驱动、@Configuration类、大量内部类      | Spring Boot + 很多的Starter                   |
| 参数设置过小           | `-XX:MaxMetaspaceSize`的值太小了     | 容器化环境的时候，资源是严格限制的，没办法无限制的申请物理内存，最后就会报错     |


## 2.4  Program Counter Register 是啥
...
一般来说，对象创建分配都会先是在年轻代经过<span class='b' style='font-size:1.5em'>15</span>次Minor GC（通常也叫Young GC）后进入到老年代。

**为啥有15次？同时Minor GC的时机是啥？**
首先，Minor GC的时机是当年轻代中的Eden区域没有足够的空间来分配新对象的时候，JVM就会立刻执行一次Minor GC。

> [!danger]
> PS： 在一些情况下，对象会直接进入老年代
> a. 比如是一个特大的对象，不管年轻代是否有足够的空间，它会直接进入到老年代中，这样就避免了Minor GC，同时也避免了Minor GC的是复制算法导致的大对象频繁移动而导致性能急剧下降
> b. Minor GC后新生代空间依旧不够，会将一些对象提前晋升到老年代中
> c. 动态年龄判断机制。 
> 	当Survivor区中相同年龄的对象大小总和大于1/2 Survivor空间，可能会将新创建的对象提前晋升到老年代
